...
i) 
ii) There is a critical point at $r=r_{c} $where the stable and unstable fixed points come together after which there  is no fixed point. This is the definition of a saddle-node bifurcation. 

iii) The bifurcation diagram looks like: 

    -  $\dot{x}=2+r x +x^{2}$ (Solution including questions 2 and 3)

    - $\dot{ x}=r-\text{ cosh }\left(x\right) $(Solution including questions 2 and 3)

    - $\dot{ x}=r+x-\text{ ln }\left(1+x\right) $(Solution including questions 2 and 3)

    -  $\dot{x}=r+\frac{x}{2}-\frac{x}{1+x}$ ($Solution$ including questions 2 and 3)
...
which you can then plug into the right hand  side of $\dot{ x}=r-\text{ cosh }\left(x\right) $ and see what happens for $x \text{ close to } 0.$
...
- Now do the same thing as questions **1** and **2** for the following transcritical bifurcations:
    ...
    - $\dot{ x}=r x-\text{ ln }\left(1+x\right)$ (Solution)

    - $\dot{ x}=x-r x \left(1-x\right)$ (Solution)

    -  $\dot{x}=x\left(r-e^{x}\right)$ (Solution)

    - $\dot{x}=r \text{ ln } x+x-1$ (Solution)
...
- Now do the same thing as questions **1** and **2** for the following pitchfork bifurcations. This time also classify them as  subcritical or supercritical.
    ...
    - $\dot{ x}=r x-\text{ sinh }\left(x\right)$ (Solution)

    - \dot{ x}=r x-4x^{3}CellChangeTimes->ExpressionUUID->10210c07-bcf7-4221-8987- 9643f8ce6c2d
...
-  For these flows on the line, find the value of r for which a bifurcation  occurs, classify the bifurcation type, and draw the bifurcation diagram 
    ...
    - \dot{ x}=r x-\frac{x}{1+x}CellChangeTimes->ExpressionUUID->7d81ca34-7e75-476a-a9f3- 0cae3a494ce8

    - \dot{ x}=5-r e^{-x^{2}}CellChangeTimes->ExpressionUUID->6d28dfa0-f3b4-4722-8cc2- 4e5612a6e039

    -  $\dot{x}=r x-\frac{x}{1+x^{2}}$

    - \dot{ x}=x+\text{ tanh }\left(r x\right)CellChangeTimes->ExpressionUUID->524d9f99-a7a8-4b44-9cde- 0f09471bc79b
